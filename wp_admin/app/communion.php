              ...
              <div class="card-body">
				...
				$currenturl="http://".$_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
				 if (isset($_GET['page_no']) && $_GET['page_no']!="") {
					...
					$result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records, DATE_FORMAT(DATE_OF_COMMUNION, '%Y') As tDate FROM tbl_communion");
					$total_records = mysqli_fetch_array($result_count);
					$total_records = $total_records['total_records'];
					$total_no_of_pages = ceil($total_records / $total_records_per_page);
					$second_last = $total_no_of_pages - 1; // total page minus 1
		
					$news = "SELECT *,COUNT(DATE_OF_COMMUNION) As tTotal, DATE_FORMAT(DATE_OF_COMMUNION, '%Y') As tDate FROM tbl_communion GROUP BY tDate DESC LIMIT $offset, $total_records_per_page"; 
